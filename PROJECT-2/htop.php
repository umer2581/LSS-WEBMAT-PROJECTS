<div class="htleft">NEWS</div>
            <div class="htcenter">
                <marquee behavior="rtl" direction="">OTP FRAUD AWARENESS PUBLIC NOTICE (6-MAY-2023)<br>
                    Staying alert and aware is the simplest way to avoid OTP Fraud Airway courier India pvt ltd do not
                    ask to share OTP/UPI pin. Any such request on phone will be treated has fraud call
                </marquee>
            </div>
            <div class="htright ht50">
                <b class="bi-facebook"></b>
                <b class="bi-twitter "></b>
                <b class="bi-youtube"></b>
                <b class="bi-phone "></b>
            </div>
            <div class="trackbtn1">
                <b class="bi bi-arrow-right-square-fill"
                    style="border: none; text-align: center; font-size: 22px; letter-spacing: 2px; color: white; float: left; margin-left: 20px;">TRACK<br>
                    SHIPPMENT
                </b>
            